...
export default function Header() {
  ...
  const SearchButton = () =>{
    ...
    const searcButtonRef = useRef<HTMLDivElement | null>(null);
    //         state: 검색 버튼 상태       //
    ...
    const onSearchWordKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>)=>{
      ...
      if(!searcButtonRef.current) return;
      searcButtonRef.current?.click();
    };
    ...
    //         effect: 검색어 path variable 변경 될때 마다 실행될 함수수    //
    useEffect(() => {
      if(searchWord){
        setWord(searchWord);
        setStatus(true);
    ...
    return(
      <div className='header-search-input-box'>
        ...
        <div ref={searcButtonRef} className='icon-button' onClick={onSearchButtonClickhandler}>
          <div className='icon search-light-icon'></div>
        </div>
      </div>
    );
  ...
  const UploadButton = () => {
    ...
    //         event handler: 업로드 버튼 클릭 이벤트 처리함수         //

...
import { GetRelationListResponoseDto } from 'apis/response/search';
...
export default function Search() {
  ...
  //          state: 관련 검색어 리스트 상태          //
  const [relativeWordList, setRelationList] = useState<string[]>([]);
  ...
  const getRelationListResponse = (responseBody: GetRelationListResponoseDto | ResponseDto | null) => {
    if(!responseBody) return;
    const {code} = responseBody;
    if(code === 'DBE') alert('데이터베이스 오류입니다.');
    if(code !== 'SU') return;
    ...
    const { relativeWordList } = responseBody as GetRelationListResponoseDto;

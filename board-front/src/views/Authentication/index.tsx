...
export default function Authentication() {
  ...
  //         state: 쿠키 상태        //
  const [cookies, setCkookie] = useCookies();
  //         function: 네비게이트 함수          //
      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            <InputBox ref={passwrodRef} label='패스워드' type={passwordType} placeholder='비밀번호를 입력해주세요.' error={error} value={password} onChange={onPasswordChangeHandler} icon={passwordButtonIcon} onButtonClick={onPasswordButtonClick} onKeyDown={onPasswordKeyDownHandler}/>
          </div>
          <div className='auth-card-bottom'>
            ...
            <div className='auth-description-box'>
              <div className='auth-description'>{'신규 사용자이신가요?'}<span className='authdescription-link' onClick={onSignUpLinkClickHandler}>{'회원가입'}</span></div>
            </div>
          </div>
        </div>
      </div>
  ...
  const SignUpCatd = () => {
    ...
    //         state: 닉네임 에러 상태        //
    const [isNicknamError, setNicknameError] = useState<boolean>(false);
    //         state: 핸드폰 번호 에러 상태        //
    const [isTelNumberError, setTelNumberError] = useState<boolean>(false);
    //         state: 주소 에러 상태        //
    const [isAddressError, setAddressError] = useState<boolean>(false);
    //         state: 개인 정보 동의 에러 상태        //
    const [isAgreddPersonalError, setAgreedPersonalError] = useState<boolean>(false);
    ...
    //         event handler: 개인 정보 동의 체크 박스 클릭릭 이벤트 처리          //
    ...
    //         event handler: 다음 단계 버튼 클릭 이벤트 처라          //
    const onNextButtonClickHandler = () => {
      ...
      if(!isEmailPattern || !isCheckdPassword || !isEqualPassword) return;
      setPage(2);
    }

    //         event handler: 회원가입 버튼 클릭 이벤트 처라          //
    const onSignUpButtonClickHandler = () => {
      ...
      if(!isEmailPattern || !isCheckdPassword || !isEqualPassword) {
        setPage(1);
        return;
      }
    ...
    //         event handler: 로그인 링크 클릭 이벤트 처라          //
    const onSignInLinkClickHandler = () => {
      setView('sign-in');
    }
    
    //         event handler: 이메일 키다운 이벤트 처라          //
    const onEmailKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
      if(event.key !== 'Enter') return;
      if(!passwordRef.current) return;
      passwordRef.current.focus();
    }
    //         event handler: 패스워드 키다운 이벤트 처라          //
    const onPasswordKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
      if(event.key !== 'Enter') return;
      if(!passwordCheckRef.current) return;
      passwordCheckRef.current.focus();
    }
    //         event handler: 패스워드 확인 키다운 이벤트 처라          //
    const onPasswordCheckKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
      if(event.key !== 'Enter') return;
      onNextButtonClickHandler();
    }
    //         event handler: 닉네임 확인 키다운 이벤트 처라          //
      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            {page === 2 && (
              <>
              <InputBox ref={nicknameRef} label='닉네임*' type='text' placeholder='닉네임을 입력해주세요.' value={nickname} onChange={onNicknameChangehandler} error={isNicknamError} message={nicknameErrorMessage} onKeyDown={onNicknameKeyDownHandler}/>
              <InputBox ref={telNumberRef} label='휴대 폰번호*' type='text' placeholder='휴대폰 번호를 입력해주세요.' value={telNumber} onChange={onTelNumberChangehandler} error={isTelNumberError} message={telNumberErrorMessage} onKeyDown={onTelNumberKeyDownHandler}/>
          ...
          <div className='auth-card-bottom'>
            ...
            <div className='auth-description-box'>
              <div className = 'auth-descriotion'>{'이미 계정이 있으신가요? '}<span className='authdescription-link' onClick={onSignInLinkClickHandler}>{'로그인'}</span></div>
            </div>
          </div> 
        </div>
      </div>
  ...
  return (
    <div id='auth-wrapper'>
      <div className='auth-container'>
        ...
        {view === 'sign-in' && <SignInCatd />}
        {view === 'sign-up' && <SignUpCatd />}
      </div>
    </div>
  )
}
